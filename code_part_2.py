...
def mark_crack(image):
    ...
    while True:
        ...
        if event_sliders == sg.WIN_CLOSED:
            ...
            img = abcd.draw_contour_on_image(image, output_image)
...
def mark_crack_method(image, alpha, beta, gamma, contour_area):
    _image = abcd.filter(image)
    _image = abcd.adjust_contrast(image=_image, alpha=alpha, beta=beta)
    _image = abcd.gamma_correction(_image, gamma)
    _image = abcd.invert_image(_image)
    _image = abcd.thresh_otsu(_image)
    contours = abcd.draw_contours(_image)
    _image = abcd.remove_contours(_image, contours, contour_area)
    return _image
